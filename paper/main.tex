...
\midrule
\texttt{0\_speed} (published STGCN) & 1 & 2.502 & 5.349 & 1.307 & 1.375 & 0.693 \\
\texttt{1\_traffic}                 & 2 & 2.369 & 5.147 & 1.250 & 1.452 & 0.731 \\
\texttt{2\_calendar}                & 5 & \textbf{2.279} & \textbf{4.861} & \textbf{1.237} & \textbf{1.489} & 0.751 \\
\texttt{3\_weather}                 & 5 & 2.485 & 5.559 & 1.302 & 1.454 & 0.696 \\
\texttt{4\_event\_geo}              & 4 & 2.318 & 5.070 & 1.255 & 1.348 & 0.756 \\
\texttt{5\_event\_geo\_att}         & 5 & 2.303 & 4.991 & 1.239 & 1.324 & 0.743 \\
\texttt{6\_all}                     & 11 & 2.361 & 5.283 & 1.406 & 1.389 & 0.739 \\
\addlinespace
...
\bottomrule
\end{tabular}
\caption{The 99 contemporaneous runs: mean over 3 seeds $\times$ 3 folds at the 60-minute horizon, MAE in mph. The three rungs at $C=5$ have near-identical parameter counts (118{,}732, 118{,}284 for \texttt{4\_event\_geo}) and differ only in what their added channels contain. Rank correlation of overall MAE with rain$\times$commute MAE is $+0.79$ ($p=0.004$), with onset recall $-0.57$ ($p=0.066$), and with propagation fidelity $-0.35$ ($p=0.298$). Onset recall and false alarms are 3-seed means throughout; they were single-seed in an earlier draft, see text.}
\label{tab:main}
\end{table*}
...
\paragraph{Weather channels make the model worse, and worse the more weather-specific the window.} \texttt{3\_weather} against \texttt{1\_traffic} --- the same model with three channels added --- is $+0.116$~mph overall ($4.9\%$ worse), $+0.171$ inside adverse weather ($6.6\%$) and $+0.412$ inside rain\,$\times$\,commute ($8.0\%$). Its onset recall drops with it, from \texttt{1\_traffic}'s $0.731$ to $0.696$, second-lowest of the eleven and below every model that does not read weather except \texttt{0\_speed} itself. The mechanism was measured before training. Controlling for time of day, rain changes neither how often breakdowns occur --- 91.8\% of commute-peak timesteps carry a shockwave in rain against 93.1\% dry --- nor how large they are (6.25 sensors involved against 6.46). Rain is a uniform capacity reduction of $-3.04$~mph, and a uniform slowdown is the easiest thing to read out of a speed history: the model does not need to be told it is raining, it can see that everything is 3~mph slower.
...
\paragraph{What aggregate error does and does not predict.} Across the eleven contemporaneous variants the three secondary metrics form a gradient rather than a dichotomy. Overall MAE ranks models almost exactly as the other \emph{point-error} metric does --- Spearman $+0.79$ ($p=0.004$) against rain\,$\times$\,commute MAE, which is unsurprising, since both are error. Against a \emph{detection} metric the relationship is moderate and not resolvable at $n=11$: $-0.57$ ($p=0.066$) against onset recall. Against the physics it is absent: $-0.35$ ($p=0.298$) against propagation fidelity. \texttt{5\_event\_geo\_att} ranks 2nd of 11 on error and 8th on propagation; \texttt{3\_weather} ranks 8th on error and 2nd on propagation. The claim this paper defends is therefore the narrow one, and it survives: a point-error metric, however carefully stratified, does not tell you whether the model learned that jams travel backwards. The cleanest case is in the next subsection.

\paragraph{The weighted loss trades false alarms for recall at no cost in MAE.} On \texttt{6\_all}, reweighting leaves aggregate MAE unchanged ($2.361\to2.356$, against a seed and fold spread of $\pm0.25$) while raising onset recall from $0.739$ to $0.780$, at 37\% more false alarms; on \texttt{0\_speed} it costs 3.8\% of aggregate MAE for a slightly larger 5.4-point recall gain, at 73\% more false alarms. Whether that trade is worth taking depends on the cost ratio, and the two users of this forecast sit at opposite ends of it. A network operator pre-deploying a signal plan pays staff hours for a false alarm and hours of egress delay for a miss, a ratio of 20--50. A navigation service pays one driver a 2--5 minute detour against a measured median 1.3 and p90 5.3 minute delay for driving into the jam, a ratio of 1--3. The operator can afford false alarms but we can evaluate that decision on only 100 egress-hour positives across the three test blocks --- 21, 79 and 0 --- while the driver cannot afford them and we can evaluate it on 46{,}525 onsets. The decision with the most tolerance for error is the one this dataset is least able to assess.
...
\toprule
Rung & Future block & \multicolumn{4}{c}{$\Delta$ MAE (future $-$ base)} & MAE@60 \\
\cmidrule(lr){3-6}\cmidrule(l){7-7}
 & & 15 & 30 & 45 & 60 & future \\
\midrule
\texttt{4\_event\_geo}      & schedule        & \textbf{--.002} & \textbf{--.006} & \textbf{--.013} & \textbf{--.021} & \textbf{2.297} \\
\texttt{5\_event\_geo\_att} & sched.+attend.\ & +.003 & +.003 & +.005 & +.006 & 2.309 \\
\texttt{2\_calendar}        & calendar        & +.002 & +.008 & +.014 & +.011 & 2.290 \\
\texttt{3\_weather}         & ECMWF forecast  & +.012 & +.020 & +.030 & +.040 & 2.526 \\
\texttt{6\_all}             & all eight       & +.012 & +.021 & +.037 & +.051 & 2.412 \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Known-future covariates, 45 further runs. Each row is its own base rung minus itself with the next 60 minutes of its knowable channels appended; negative is better. Only \texttt{4\_event\_geo} improves, and it does so in all three folds at all four horizons. Pooled over rungs and folds the intervention is \emph{harmful} at every horizon ($p=0.030,0.020,0.023,0.051$). Differences are formed from unrounded fold means and so may differ by $0.001$ from subtracting the last column. These rows are deliberately \emph{not} merged into Table~\ref{tab:main}: a lookahead model receives strictly more information than any model there, so ranking the two regimes in one column would invite exactly the across-row reading this paper argues against. The paired difference, not the absolute level, is what this experiment estimates.}
\label{tab:future}
\end{table}
...
Figure~\ref{fig:prop} answers Section~\ref{sec:propagation}'s question. Observed shockwave onsets travel upstream $1.68\times$ more often than downstream at a 15-minute lag. Every contemporaneous Chebyshev model reproduces between 78\% and 89\% of that peak at the 60-minute horizon ($1.31$ to $1.49$ against the observed $1.68$) and none collapses to 1.0, so all are learning a travelling wave rather than a smooth conditional mean --- but none reaches the truth either, and the ranking is uncorrelated with MAE.
...
\paragraph{Single-seed detection metrics cannot rank models.} Onset recall and false alarms in Table~\ref{tab:main} are 3-seed means. They were not in an earlier version of our own evaluation code, which grouped runs by a seed pattern anchored to the end of the filename and then assigned rather than accumulated, so seven rows silently reported one seed while presenting as three. The correction is small per row but not negligible in aggregate: it moved \texttt{4\_event\_geo} from $0.763$ to $0.756$ and its false alarms from $2{,}144$ to $1{,}936$, and it changed the rank correlation between aggregate MAE and onset recall from $-0.65$ ($p=0.032$) to $-0.57$ ($p=0.066$), i.e.\ from significant to not. Measured across the runs where all three seeds survived, recall varies by up to $0.067$ between seeds of one model on one fold, against a spread of about $0.09$ across the whole column. A detection metric reported from a single seed on this corpus is therefore not interpretable as a ranking, and we flag which rows were affected in the released table.
